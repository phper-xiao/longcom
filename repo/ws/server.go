...
func (u HTTPUpgrader) Upgrade(
	r *http.Request,
	w http.ResponseWriter,
) (conn net.Conn, rw *bufio.ReadWriter, hs Handshake, err error) {
...
func (u Upgrader) TakeUpgradeHTTPHeader(in []byte) (readoffset int, header []byte, err error) {
	// Read HTTP request line like "GET /ws HTTP/1.1".
	buf := in
	idx := bytes.Index(buf, []byte("\r\n\r\n"))
	if idx == -1 {
		// 没找到HTTP完整头部，返回需要更多数据的错误
		err = merrors.ErrGnetWebsocketCodecNeedMoreData
		return
	}

	header = buf[:idx]
	// 读取的 offset 要加上 len([]byte("\r\n\r\n"))
	readoffset = idx + 4
	return
}
func (u Upgrader) Upgrade(in []byte) (out []byte, hs Handshake, err error) {
	...
	case err == nil && headerSeen != headerSeenAll:
		...
		case headerSeen&headerSeenSecKey == 0:
			err = ErrHandshakeBadSecKey
		default:
			err = errors.New("unknown headers state")
		}
	...
	if err != nil {
		...
		bw := bufio.NewWriter(&bytesBuffer)

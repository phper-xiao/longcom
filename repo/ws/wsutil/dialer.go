...
func (d *DebugDialer) Dial(
	ctx context.Context,
	urlstr string,
) (conn net.Conn, br *bufio.Reader, hs ws.Handshake, err error) {
...
// Read rwConn 实现 Read 接口
func (rwc rwConn) Read(p []byte) (int, error) {
	return rwc.r.Read(p)
}

// Write rwConn 实现 Write 接口
...
// Read prefetchResponseReader 实现 Write 接口

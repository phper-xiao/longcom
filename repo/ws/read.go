...
// ReadHeaderFromBytes reads a frame header from in.
func ReadHeaderFromBytes(in []byte) (readoffset int, h Header, err error) {
	...
	// So 14 - 2 = 12.
	if len(in) < 2 {
		err = merrors.ErrGnetWebsocketCodecNeedMoreData
		return
	}
...
// ReadFrameFromBytes reads a frame from in.
func ReadFrameFromBytes(in []byte) (readoffset int, frameBytes []byte, err error) {
	var header Header
	var headerReadOffset int

	headerReadOffset, header, err = ReadHeaderFromBytes(in)

	if err != nil {
		return
	}
	i64headerReadOffset := int64(headerReadOffset)

	rest := in[headerReadOffset:]

	// TODO: 限制 header.Length 大小为 2 M

	if header.Length > 0 {
		if int64(len(rest)) < header.Length {
			// 剩余数据比 Header 少，还需要再读入数据
			err = merrors.ErrGnetWebsocketCodecNeedMoreData
			return
		}
		...
		frameBytes = in[:i64headerReadOffset+header.Length]

		readoffset = int(i64headerReadOffset + header.Length)
	}
